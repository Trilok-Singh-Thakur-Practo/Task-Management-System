...
public class TaskController {
    ...
    public ResponseEntity<List<TaskResponse>> getAllTasks(@RequestHeader("Authorization") String authHeader) {
        ...
        // For simplicity, we're using the username as user ID; in a real application, 
        // you would do a lookup to get the actual user ID from the username
    ...
    public ResponseEntity<List<TaskResponse>> getOverdueTasks(
        ...
        List<TaskResponse> taskResponses = tasks.stream()
                .filter(task -> task.getStatus() != TaskStatus.DONE) // Only include non-completed tasks
                .map(this::convertToDto)
                .collect(Collectors.toList());
